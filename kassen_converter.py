    ...
    try:
        with zipfile.ZipFile(path) as zf:
            ...
            if "document.xml" not in names:
                return False
            candidates = [n for n in names if n.lower().endswith(".xml") and n != "document.xml"]
            if not candidates:
                return False
...
def validate_extf_zip(path: Path) -> bool:
    try:
        with zipfile.ZipFile(path) as zf:
            ...
            if not matches:
                return False
            text = zf.read(matches[0]).decode("cp1252")
    ...
    except (zipfile.BadZipFile, UnicodeDecodeError, csv.Error, KeyError):
        return False
...
def is_card_label(label: str) -> bool:
    text = " ".join(label.casefold().strip().split())
...
def parse_extf(path: Path, receipts: dict[str, Receipt], logger: logging.Logger) -> None:
    ...
    rows = csv.reader(lines[2:], delimiter=";", quotechar='"')
    for row in rows:
        ...
        if not receipt_id:
            continue
        amount_text = row[index["Umsatz (ohne Soll/Haben-Kz)"]].strip()
        if not amount_text:
            continue
        try:
    ...
    for receipt in receipts.values():
        ...
        receipt.payment_sum = (receipt.cash + receipt.card + receipt.other_payment).quantize(Decimal("0.01"))
    ...
    with path.open("w", encoding="utf-8-sig", newline="") as fh:
        ...
        for receipt in sorted(receipts.values(), key=lambda r: (datetime.strptime(r.date, "%d.%m.%Y"), r.time, r.receipt_id)):
            for p in receipt.positions:
                writer.writerow([
                    p.date, p.time, p.receipt_id, p.item,
                    german_money(p.amount), german_money(p.vat), p.account, p.payment_method,
                ])
    return path
...
def write_receipt_csv(directory: Path, period: str, receipts: dict[str, Receipt]) -> Path:
    ...
    columns = [
        "Datum", "Uhrzeit", "Bonnummer", "Brutto gesamt", "Brutto 7 %",
        "Brutto 19 %", "Trinkgeld", "Bar", "Karte", "Sonstige Zahlart",
        "Zahlungsart Summe", "Differenz", "Pruefhinweis",
    ]
    with path.open("w", encoding="utf-8-sig", newline="") as fh:
        writer = csv.writer(fh, delimiter=";", quotechar='"', quoting=csv.QUOTE_MINIMAL)
        writer.writerow(columns)
        for r in sorted(receipts.values(), key=lambda x: (datetime.strptime(x.date, "%d.%m.%Y"), x.time, x.receipt_id)):
            writer.writerow([
                r.date, r.time, r.receipt_id, german_money(r.total), german_money(r.vat_7),
                german_money(r.vat_19), german_money(r.tips), german_money(r.cash),
                german_money(r.card), german_money(r.other_payment), german_money(r.payment_sum),
                german_money(r.difference), " | ".join(r.warnings),
            ])
    return path
...
def run(directory: Path | None = None) -> tuple[Path, Path]:
    ...
    if extf_path:
        ...
        for receipt in receipts.values():
            receipt.warnings.append("EXTF-Kontrollquelle fehlt")
